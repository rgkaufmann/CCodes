/*
 ...
 Author:		Logan Zhang and Micaela Peragallo
 Student #s:	41872169 and 61376166
 CS Accounts:	z5o1b and t3k1b
...
void analyze_segments( char * sample_segment, char ** candidate_segments, int number_of_candidates )
{
	...
	for (i = 0; i < number_of_candidates; ++i) {
		...
		printf("Candidate number %d is matched score: %d\n", i + 1, score);
	}
...
int calculate_score( char * sample_segment, char * candidate_segment)
{
	...
	int k,h;
	int temp;
	char* temp_sample = (char*)malloc(3*sizeof(char));
	char* temp_candidate = (char*)malloc(3 * sizeof(char));
	...
	iterations =  candidate_length_in_codons - sample_length_in_codons + 1;
	temp = sample_length % 3;

	for (i = 0; i < iterations ; i++){
		store = 0;
		for (j = 0; j < sample_length - temp; j = j+3) {
			h = 0;
			for (k = j; k < j + 3; k++) {
				
				temp_sample[h] = sample_segment[k];
				temp_candidate[h] = candidate_segment[k + i * 3];
					h++;
			}
			temp_score = check_codons(temp_sample, temp_candidate);
			store = store + temp_score;
			}
		if (score < store) {
			score = store;
		}
...
/*
* Checks if two codons correspond to the same amino acid
*
* PARAM:     Pointer to a null-terminated string representing the fist codon
* PARAM:     Pointer to a null-terminated string representing the second codon
* PRE:       The two strings are valid codons
* POST:      NULL
* RETURN:    1 if codons correspond to the same amino, 0 if they don't
*/
int same_amino (char* string1, char* string2){
	int row1;
	int row2;

	row1 = find_row(string1);
	row2 = find_row(string2);

	if (strcmp(codon_names[row1], codon_names[row2]) == 0) {
		return 1;
	}
	return 0;
}
...
/*
* Finds the row in which a codon is located inside the codon_codes array in dna.h
*
* PARAM:     Pointer to a null-terminated string representing the codon
* PRE:       String is a valid codon
* POST:      NULL
* RETURN:    Number of the row it is located in, -1 if it's not found (error)
*/
int find_row(char* string) {
	int row;
	for (row = 0; row < NUMBER_OF_CODONS; row++) {
		if (strncmp(codon_codes[row], string, CODON_LENGTH) == 0) {
			return row;
		}
	}
	return -1;
...
/*
* Scores the combination of two codons (using the same_amino and check_nucleo functions)
*
* PARAM:     Pointer to a null-terminated string representing the first codon
* PARAM:	 Pointer to a null-terminated string representing the second codon
* PRE:       Strings correspond to valid codons
* POST:      NULL
* RETURN:    Score computed
*/
int check_codons(char* string1, char* string2) {
	int score = 0;
	if (strncmp(string1, string2, CODON_LENGTH) == 0) {
		score = 10;
	}
	else if(same_amino(string1, string2)){
			score = 5;
	}
	else { 
		score = check_nucleo(string1, string2);
	}
...
/*
* Scores the combination of two codons by its nucleotide pairings
* Computes same nucleotide matches
* Uses the function match_nucleo function to compute nucleotide base pair matches
* Scores the nucleotide pairings accordingly, adds to the total score
*
* PARAM:     Pointer to a null-terminated string representing the first codon
* PARAM:	 Pointer to a null-terminated string representing the second codon
* PRE:       Strings correspond to valid codons
...
* RETURN:    Score computed
*/
int check_nucleo(char* string1, char* string2) {
	int score = 0;
	int i;
	...
	for (i = 0; i < CODON_LENGTH; i++) {
		...
		else if (match_nucleo(string1[i], string2[i])) {
			score++;
...
/*
* Checks if two nucleotides belong to a matching base pair
*
* PARAM:     Pointer to a null-terminated string representing the first nucleotide
* PARAM:	 Pointer to a null-terminated string representing the second nucleotide
* PRE:       Strings correspond to valid nucleotides
* POST:      NULL
* RETURN:    1 if they match, 0 if they don't
*/
int match_nucleo(char ch1, char ch2) {
	if (ch1 == 'A' && ch2 == 'T' || ch1 == 'T' && ch2 == 'A') {
		return 1;
	}

	else if (ch1 == 'C' && ch2 == 'G' || ch1 == 'G' && ch2 == 'C') {
		return 1;
	}
